...
const Saveforlaterbtn = ({card}:{card:Icard}) => {
  ...
  const {saveexercise, setSavedexercise} = useContext(ExerciseContext) as {
    ...
    const handlesave = ()=> {
      const alreadyAdded = saveexercise.find( (exercise) => exercise.id === card.id ); if (alreadyAdded) { toast.error("You already Saved this exercise!"); return; }
  ...
  return (
    <div>
      <button onClick={()=>handlesave()} className="btn bg-[#161922] hover:bg-[#1f2430] text-white border border-white/10 flex-1 rounded-xl">
